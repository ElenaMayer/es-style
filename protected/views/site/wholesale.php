<div class="order">

    <h1>Одежда оптом</h1>

    <a class="hint hint-link" href="<?= Price::model()->getPrice()?>">
        <div class="hint__icon-price"></div><span class="hint__title">Скачать прайс</span>
    </a>
    <h3>Условия сотрудничестваа</h3>
    <ul class="list list_shopping">
        <li class="list__item">Мы работаем с физическими и юридическими лицами, с организаторами совместных покупок.</li>
        <li class="list__item">Минимальная оптовая партия - <b>5000 руб.</b></li>
        <li class="list__item">Заказ можно сделать через приведенную ниже форму или позвонив по телефону <b><?=Yii::app()->params['mainPhone']?></b>.</li>
    </ul>

    <h3>Доставка</h3>
    <ul class="list list_shopping">
        <li class="list__item">Доставка по Новосибирску - бесплатно.</li>
        <li class="list__item">Доставка в регионы России осуществляется Почтой России или любой удобной для Вас транспортной компанией.</li>
        <li class="list__item">Доставка до транспортной компании - бесплатно.</li>
    </ul>

    <h3>Оплата</h3>
    <ul class="list list_shopping">
        <li class="list__item">При заказе из Новосибирска возможет наличный и безналичный расчет.</li>
        <li class="list__item">При заказе по России - безналичный расчет, предоплата 100%.</li>
    </ul>
    <h3>Заказ</h3>
    <?php $this->renderPartial('_wholesale_form',array('model'=>$model)); ?>
</div>
<?php $this->renderPartial('_size_tab'); ?>
